...
function buildCapabilityPrompt() {
    const tokenLines = Object.entries(TOKENS)
        .map(([id, def]) => `${id}: ${def.label} (${def.kind === 'color' ? 'value must be a hex color like #8b5cf6' : `value must be a size in px, ${def.min}-${def.max}`})`)
        .join('\n');
...
- "actions" is the list of changes to make. Use an empty array if the visitor is just asking a question, is off-topic, or is asking for something outside this manifest.
